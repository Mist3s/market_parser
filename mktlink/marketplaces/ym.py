...
def classify_response(html: str, *, anchor_ids: set[str], status: int = 200) -> Verdict | None:
    ...
    if status in (301, 302, 303, 307, 308):
        # ЗАМЕР 2026-09-03 и повторно 2026-09-04: карточка Я.Маркета отвечает
        # ``302`` РОВНО в одном случае — увод на ``/showcaptcha``. По этому
        # редиректу лежит страница «Вы не робот» с маркерами ``smartcaptcha``
        # и ``checkcaptcha``.
        #
        # Раньше эта ветка отсутствовала, и путь был такой: тело у 302 пустое,
        # значит текстовых маркеров нет, значит ``identity()`` возвращает
        # ``None``, значит вердикт ``SILENT_EMPTY``. На деньги это не влияло —
        # оба вердикта в ``CHARGES_PROXY``, — но оператору сообщалось «страница
        # пришла пустая» вместо «нас челленджнули», то есть причину искали бы
        # в разметке вместо адреса.
        #
        # Оговорка честная: отличить челлендж от безобидного нормализующего
        # редиректа здесь НЕЛЬЗЯ — заголовок ``Location`` до классификатора не
        # доезжает, ``Response`` его не несёт. Если Я.Маркет однажды начнёт
        # нормализовать URL редиректом, эту ветку придётся кормить заголовком.
        return Verdict.CAPTCHA
