"""Нормализация текста и денег. СКОПИРОВАНО из удалённого скрейпера
(``841dac3``, ``market_parser/normalize.py``).

Копия, а не импорт, и на момент копирования это было осознанным решением:
привязка сервиса к релизному циклу скрейпера означала бы, что правка в
парсере детского питания меняет ответ этого API. Сам скрейпер удалён, но
копия остаётся правильной формой — взято только то, что действительно чистое
и универсальное, и взято дословно, чтобы поведение можно было сверить с
историей.
