...
class ProductRequest(BaseModel):
    ...
    #: Сходить в маркетплейс, ИГНОРИРУЯ кэш.
    #:
    #: Появилось вместе с суточным сроком свежести и является его условием.
    #: Пока кэш жил 15 минут, «подожди и обновится» было честным ответом на
    #: любое сомнение в свежести. При сутках — уже нет: клиент, которому
    #: нужен продавец СЕЙЧАС, не имел никакого способа его получить, потому
    #: что ``max_stale_s`` разрешает старое, но не запрещает его.
    #:
    #: Стоит вчетверо дороже холодного запроса (``COST['force_refresh']``)
    #: и сознательно карательно: сброс кэша — самый дешёвый способ устроить
    #: нам DoS чужими руками, а с платным скрейпинг-API ещё и способ сжечь
    #: месячную квоту кредитов.
