...
#: Лестница для маркетплейсов, идущих через скрейпинг-API. ОДНА ступень, и это
#: не упрощение, а следствие того, чем платят.
#:
#: Через собственный адрес вторая ступень имеет смысл: она пробует другой
#: транспорт того же маркетплейса, и стоит она только времени. Через чужой
#: API вторая ступень — это второй списанный кредит за тот же самый запрос к
#: тому же поставщику, который ротацию адресов и повторы делает у себя внутри.
#: Платить дважды за одно и то же нечем оправдать.
#:
#: ЗАМЕРЕНО 2026-09-03. Разброс здесь важнее среднего, поэтому наблюдения
#: перечислены целиком, а не сведены в одно число:
#:
#: * Я.Маркет (без рендеринга) — 4 621, 6 301, 6 732 и **более 8 000** мс.
#:   Первые два сэмпла дали потолок 8 000, и следующий же живой прогон в него
#:   не уложился: клиент получил ``202`` на запросе, который работал. Урок
#:   записан здесь, а не в истории коммитов: **два наблюдения не выборка**, и
#:   потолок по ним ставить нельзя. Текущий 12 000 — самое медленное
#:   наблюдение с запасом.
#: * Ozon (с рендерингом на стороне поставщика) — 9 098, 9 834, 10 754,
#:   13 178 и 25 633 мс. **В потолок 15 с укладывается не всегда.** Пол 9 000
#:   поставлен по самому быстрому замеру: ступень с таким полом не допускается
#:   в бюджет 5 с вовсе, и это правильный отказ вместо гарантированного
#:   таймаута. Настоящее решение для Ozon — не потолок пошире, а отложенная
#:   работа через уже существующий ``202``/``job_id``; до неё честный исход
#:   медленного ответа — ``BUDGET_EXHAUSTED``, то есть «наша вина», без
#:   штрафа адресу.
#:
#: Потолки выведены из ``net_ms`` при бюджете 30 с, и у Ozon это сознательный
#: компромисс, который стоит назвать вслух.
#:
#: ``net(30000, ozon, 0) = 29 215``. Ступени нужно покрыть самое медленное
#: наблюдение (25 633 мс) И полторы секунды сокращения ссылки, которые
#: транспорт вычитает из её окна при промахе кэша коротких ссылок
#: (``SHORTEN_CAP_MS``). Арифметика: 25 633 + 1 500 = 27 133, округлено до
#: **27 150**. На промахе кэша коротких ссылок поставщик видит 25 650 мс,
#: на попадании — все 27 150.
#:
#: Здесь стояло 27 000, и это была ошибка ровно в том случае, ради которого
#: потолок и поднимали: на промахе поставщик получал 25 500 мс — на 133 мс
#: меньше самого медленного наблюдения. Докстрока называла цель, которой
#: число не достигало.
#:
#: Чем за это платим: резидуал становится 2 065 мс, то есть МЕНЬШЕ гарда
#: спейсинга ``MIN_INTERVAL_MS["ozon"] = 5000``. По правилу планировщика это
#: ровно то условие, при котором возможен ``202`` из-за ожидания слота. Взято
#: осознанно: слот занят только если предыдущий запрос к Ozon был менее пяти
#: секунд назад, а при заявленном потоке 0–10 ссылок в СУТКИ это событие
#: пренебрежимо. Обратный выбор (ceil 24 215 ради полного гарда) стоил бы
#: непокрытия самых медленных наблюдений — то есть гарантированного ``202``
#: вместо маловероятного.
API_LADDER: dict[str, tuple[Rung, ...]] = {
    "ozon": (Rung("ozon.api_html", "replay", 9_000, 27_150),),
    # У Я.Маркета запаса хватает без компромиссов: 4.6–8.0 с при net 29 075,
    # поэтому потолок оставляет полный гард спейсинга и ещё половину бюджета.
    "ym": (Rung("ym.api_html", "replay", 4_500, 12_000),),
}
...
def request_ladder(
    mp: str, *, with_render: bool = False, via_api: bool = False
) -> tuple[Rung, ...]:
    """Ступени, допустимые на пути запроса: без минтинга, и по умолчанию без рендера.

    ``via_api`` полностью заменяет лестницу на :data:`API_LADDER`, а не
    дополняет её: через скрейпинг-API идут ДРУГИЕ запросы с другой ценой и
    другим временем, и смешивать их со ступенями собственного егресса
    значит расписать бюджет по числам, которых в этом режиме не бывает.
    Рендер при этом делает поставщик, поэтому ``with_render`` здесь
    неприменим и игнорируется.
    """
    if via_api:
        return API_LADDER[mp]
...
def plan(
    budget_ms: int, mp: str, hops: int, *, with_render: bool = False, via_api: bool = False
) -> Plan:
    ...
    ladder = request_ladder(mp, with_render=with_render, via_api=via_api)
    if not ladder:
